...
<head>
    ...
    <title>@yield('title', 'Portal Dosen') - Pra-TA Kriya</title>
    ...
    <style>
        :root {
            /* Kita gunakan warna dasar yang senada namun bisa dibedakan jika diinginkan */
            --primary-dark: #0f172a; 
    ...
    <div class="d-flex">
        <nav id="sidebar" class="d-flex flex-column shadow-sm">
            <a href="{{ route('dosen.dashboard') }}" class="sidebar-brand">
                <div class="brand-icon" style="background-color: #1e293b;"><i class="bi bi-person-workspace"></i></div>
            ...
            <div class="p-3 overflow-y-auto">
                ...
                <ul class="nav nav-pills flex-column mb-auto">
                    ...
                    <li class="nav-item mt-3">
                        <div class="small text-muted fw-bold text-uppercase tracking-wide mb-2 px-3" style="font-size: 0.7rem; letter-spacing: 1px;">Dosen Pembimbing</div>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('dosen.topik.index') }}" class="nav-link {{ request()->routeIs('dosen.topik.*') ? 'active' : '' }}">
                            <i class="bi bi-journal-bookmark-fill"></i> Topik Interest Saya
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('dosen.review.index') }}" class="nav-link {{ request()->routeIs('dosen.review.*') ? 'active' : '' }}"">
                            <i class="bi bi-person-lines-fill"></i> Review Pengajuan
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('dosen.bimbingan.index') }}" class="nav-link {{ request()->routeIs('dosen.bimbingan.*') ? 'active' : '' }}">
                            <i class="bi bi-person-lines-fill"></i> Bimbingan
                        </a>
                    </li>
                    
                    {{-- 
                        CATATAN PENTING: Sesuaikan 'is_admin' di bawah ini dengan nama kolom 
                        di tabel database 'dosens' Anda yang menandakan dia adalah Admin Prodi.
                        Contoh lain: $user->role === 'admin' atau $user->is_admin_prodi 
                    --}}
                    @if(Auth::guard('dosen')->user()->is_admin === 'YES')
                        <li class="nav-item mt-4">
                            <div class="small text-muted fw-bold text-uppercase tracking-wide mb-2 px-3" style="font-size: 0.7rem; letter-spacing: 1px;">
                                <i class="bi bi-shield-lock-fill text-warning me-1"></i> Modul Admin Prodi
                            </div>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('dosen.prodi.periode.index') }}" class="nav-link {{ request()->routeIs('dosen.prodi.periode.*') ? 'active' : '' }}">
            ...
            <nav class="navbar navbar-expand-lg navbar-glass py-3 px-4">
                <div class="container-fluid p-0 d-flex justify-content-between align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        @php $user = Auth::guard('dosen')->user(); @endphp
                        <button class="btn user-profile-btn dropdown-toggle border-0" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            @else
                                <div class="user-avatar" style="background-color: #1e293b;">{{ substr($user->nama, 0, 1) }}</div>
                            @endif
                            <span class="d-none d-sm-inline">{{ $user->nama }}</span>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 mt-2 p-2" style="min-width: 220px;">
                            <li class="px-3 py-2">
                                <span class="d-block fw-bold text-dark text-truncate">{{ $user->nama }}</span>
                                <span class="d-block small text-muted">NIP: {{ $user->nip ?? '-' }}</span>
